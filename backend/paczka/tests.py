...
class CourseModelTest(TestCase):
    ...
    def test_lecturer_label(self):
        leader = Course.objects.get(id=1)
        field_label = leader._meta.get_field('lecturer').verbose_name
...
class MaterialModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        Material.objects.create(author_name='Rob',
        author_surname='Marshall',
        author_email='rob.marshall@pirate.bay',
        path_to_file='/tmp',
        descripton='good director',
        upload_date='2022-10-23',
        course=c)

    def test_string_representation_of_object(self):
        leader = Material.objects.get(id=1)
        expected_str = f'Uploaded by {leader.author_name} {leader.author_surname}({leader.author_email}) \
at 2022-10-23: {leader.descripton}'
        self.assertEqual(str(leader),expected_str)
